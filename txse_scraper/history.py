...
FIELDS = [
    ...
    "market_share_pct","lit_market_share_pct","total_us_volume","lit_us_volume",
...
def _is_off_exchange(name: str) -> bool:
    u = (name or "").upper()
    return "TRF" in u or "FINRA" in u
...
def extract(rows, start: date, end: date):
    ...
    for r in rows:
        ...
        if not start <= d <= end:
            continue
        shares = num(r.get("Total Shares")) or 0
        consolidated_totals[d] = consolidated_totals.get(d, 0.0) + shares
        if not _is_off_exchange(r.get("Market Participant", "")):
            lit_totals[d] = lit_totals.get(d, 0.0) + shares

    out = []
    for r in rows:
        if r.get("Market Participant") != TXSE:
            continue
        d = date.fromisoformat(r["Day"])
        if not start <= d <= end:
            continue
        sv, dv, tc = num(r["Total Shares"], True), num(r["Total Notional"]), num(r["Total Trade Count"], True)
        total_us = consolidated_totals.get(d)
        lit_us = lit_totals.get(d)
        out.append({
            "trade_date": d.isoformat(), "share_volume": sv, "dollar_volume": dv, "trade_count": tc,
            "market_share_pct": (sv / total_us * 100) if sv and total_us else None,
            "lit_market_share_pct": (sv / lit_us * 100) if sv and lit_us else None,
            "total_us_volume": total_us, "lit_us_volume": lit_us,
...
def upsert(records, path: Path):
    ...
    for rec in records:
        existing[rec["trade_date"]] = {k:"" if rec.get(k) is None else str(rec.get(k)) for k in FIELDS}
...
def main(argv=None):
    ...
    for year in range(start.year,end.year+1):
        records += extract(fetch_year(year),start,end)
